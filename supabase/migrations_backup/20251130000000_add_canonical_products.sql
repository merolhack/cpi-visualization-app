-- Create cpi_canonical_products table
...
-- Add RLS to cpi_canonical_products
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE tablename = 'cpi_canonical_products' 
        AND policyname = 'Allow public read access on cpi_canonical_products'
    ) THEN
        CREATE POLICY "Allow public read access on cpi_canonical_products"
            ON public.cpi_canonical_products
            FOR SELECT
            USING (true);
    END IF;
END $$;

-- Add canonical_id to cpi_products
ALTER TABLE public.cpi_products 
ADD COLUMN IF NOT EXISTS canonical_id BIGINT REFERENCES public.cpi_canonical_products(canonical_id);

-- Add index for performance
CREATE INDEX IF NOT EXISTS idx_cpi_products_canonical_id ON public.cpi_products(canonical_id);
...
-- Update add_product_and_price RPC to handle canonical_id
